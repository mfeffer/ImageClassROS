...
def getAccuracy(k, n_nn, added_limit, hue_multi=0, sat_multi=0):
    ...
    pTestSuccess = float(test_successes)/(test_successes+test_failures)
    pTrainingSuccess = float(training_successes)/(training_successes+training_failures)
    print "Test Success: " + str(pTestSuccess)
    print "Training Success: " + str(pTrainingSuccess)
    print test_results
    return pTestSuccess, pTrainingSuccess, test_results, training_results
...
for s in sal:
    include_salient = s
    pTestSuccess, pTrainingSuccess, test_results, training_results = getAccuracy(k, n_nn, added_limit)
    ps.append(pTestSuccess)
    pstraining.append(pTrainingSuccess)
